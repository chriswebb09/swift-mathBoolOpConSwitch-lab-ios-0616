...
class VCTests: QuickSpec {
	...
	override func spec() {
		...
		describe("averageIsAbove75(_:b:c:)") {
			...
			expect( testVC.averageIsAbove75( 76, b: 75, c: 71)).to(equal(false))    // NOTE we might want to get specific about the precision & rounding they should use (eg. is 75.4 > 75?  75.4 is *definitely* greater than 75.0)
			
			expect( testVC.averageIsAbove75( 75, b: 75, c: 75 )).to(equal(false))    // must be *OVER* 75

			expect( testVC.averageIsAbove75( 75, b: 75, c: 75)).to(equal(false))   // they should be using Doubles, so this is *just* over 75

			expect( testVC.averageIsAbove75( 80, b: 80, c: 80 )).to(equal(true))
			}
		}
		...
		describe("passwordCombo(_:password:)") {
			it("Takes a String and an Int.  If the String is equal to Jerry, Elaine or Michael, AND the Int is evenly divisible by 3, then it returns true, otherwise it returns false") {
                ...
                expect(testVC.passwordCombo(username: "Jerry", password: 3)).to(equal("Access Denied"))

                expect(testVC.passwordCombo(username: "Elaine", password: 6)).to(equal("Access Denied"))
				
                expect(testVC.passwordCombo(username: "Michael", password: 21)).to(equal("Access Denied"))
		...
		describe("doIt(_:b:)") {
			...
			it("Checks the validity of each paramters and returns their product as a double or, if there's a validation error, it returns 0") {
                ...
                expect(testVC.doIt("3", b:12)).to(equal(0.0))
				
                expect(testVC.doIt("3", b:12)).to(equal(0.0))  // make sure they're keeping precision
                ...
                expect(testVC.doIt("3", b:8)).to(equal(0.0))  // other arg out of range
